...
void loop() {
  ...
  if (millis() - time_100Hz >= 10) {
    ...
    // 100Hzで実行する処理
      // BNO055から100Hzで測定
      if (acceljudge == 0) {
        if (mediumBNO(event) > 46) {//中央値が閾値を超えた回数カウント
          accelcount++;
        } else {
          accelcount = 0;  //初期化
        }
      ...
      if (acceljudge == 1) {
        if (mediumBNO(event) < 10) {//中央値が閾値を下回った回数カウント
          accelcount++;
        } else {
          accelcount = 0;  //初期化
        }
    ...
    // BME280から100Hzで測定
      if (altitudejudge == 0) {
          if (altitudecount > 5) {  //5回連続で閾値を超えたときの処理
        ...
        if (altitudejudge == 1) {
          if (altitudecount > 5) {  //5回連続で閾値を下回ったときの処理
...
double mediumBME(){
  ...
  for (int i = 0; i < 10; i++) {
    ...
    BME[i] = bme.readAltitude(SEALEVELPRESSURE_HPA);//高度値取得
  }
  for (int i = 0; i < 9; i++) {
    for (int j = 1; j < 9; j++) {
      if (BME[j - 1] < BME[j]) {
